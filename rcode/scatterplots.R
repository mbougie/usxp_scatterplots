...
query_usxp_exp = read_file(paste(path_sql, eu, figure_obj$datasets_usxp$exp$query, sep = "\\"))
df_usxp_exp <- createDF_usxp(query=query_usxp_exp)


query_usxp_net = read_file(paste(path_sql, eu, figure_obj$datasets_usxp$net$query, sep = "\\"))
df_usxp_net <- createDF_usxp(query=query_usxp_net)
...
###############################################################################################
  ...
  for(name in names(figure_obj$datasets)){
    print(name)
    con=eval(parse(text = figure_obj$datasets[[name]]$con))
    usxp_query=eval(parse(text = figure_obj$datasets[[name]]$usxp_query))
    query = read_file(paste(path_sql, eu, figure_obj$datasets[[name]]$query, sep = "\\"))
