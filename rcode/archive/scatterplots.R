...
### stacked these datasets
y_axis_dfs = dplyr::bind_rows(df_nri, df_nlcd, df_nass)

####inner join these datafraems
y_axis_dfs_w_states <- y_axis_dfs %>% inner_join(df_states, by = "state_name")

y_axis_dfs_w_states$perc = (y_axis_dfs_w_states$acres_per_year/y_axis_dfs_w_states$acres_calc * 100)
...
df_usxp <- dbGetQuery(con_usxp_deliverables, query_usxp)

df_usxp_w_states <- df_usxp %>% inner_join(df_states, by = "state_name")
df_usxp_w_states$usxp_perc = (df_usxp_w_states$acres_usxp_per_year/df_usxp_w_states$acres_calc * 100)
 
###join datasets together 
df <- df_usxp_w_states %>% inner_join(y_axis_dfs_w_states, by = "state_name")
