...
# df_nri = createDF(con=con_nri, query=query_nri, sat=c(-5000, 5000))
df_nri = createDF(con=con_nri, query=query_nri, sat=c(-100, 1e30))
...
h_nri = ggplot(data=df_nri, aes(perc_sat)) + geom_histogram(bins = 1000) + theme(legend.text=element_text(size=20))
...
df_nlcd = createDF(con=con_usxp_deliverables, query=query_nlcd, sat=c(-100, 1e30))
# df_nlcd = na.omit(df_nlcd)
...
df_nass = createDF(con=con_nass, query=query_nass, sat=c(-500, 500))
...
h_nass = ggplot(data=df_nass, aes(perc_sat)) + geom_histogram() + theme(legend.text=element_text(size=20))
h_nass
...
fileout = paste0('I:\\projects\\usxp\\series\\s35\\deliverables\\scatterplot\\acatterplot_perc.png')
